...
<script>
    ...
    function renderMenuItems() {
        if (!selectedMenu) {
            menuPanel.innerHTML = 'একটি মেনু সিলেক্ট করুন…';
            return;
        }
        const itemList = selectedMenu.items.map((it, i) =>
            `<li class="list-group-item d-flex justify-content-between align-items-center">${it.label}<span class="text-secondary small">${it.url||''}</span></li>`
            ).join('');
        menuPanel.innerHTML = `
        ...
        if (addBtn) {
            addBtn.addEventListener('click', () => {
                const l = document.getElementById('miLabel').value.trim();
                const u = document.getElementById('miUrl').value.trim();
                if (!l) return;
